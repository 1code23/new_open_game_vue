...
export default {
  ...
  methods: {
    ...
    async getPageConfigDetail(id) {
      const res = await this.$api.getPageConfigDetail({ id });
      const { list = [] } = res;
      // res_type值
      // 100101 单icon横向自营应用
      // 100102 单icon横向应用宝应用
      ...
      this.selectList = list.map((item) => {
        ...
        if (item.res_type == 100101) {
          ...
          if (item.external_res_info) {
            ...
            obj.hasLabel = false; //不显示标签字段
          }
        } else {
          if (item.ass_id) {
            // 组件
            obj.sourceType = "assembly";
            obj.res_type = item.res_type;
            obj.assembly_name = item.res_name;
            obj.assembly_id = item.ass_id;
          } else {
            // 微信小程序API
    ...
    async pageDetailAdd() {
      const list = this.selectList.map((item, index) => {
        ...
        if (item.sourceType == "app") {
          ...
          obj.order_id = item.order_id;
          obj.apk_id = item.apk_id;
        } else if (item.sourceType == "assembly") {
          // 组件
          obj.res_type = item.res_type;
          obj.res_name = item.assembly_name;
          obj.ass_id = item.assembly_id;
      ...
      const data = { page_id: this.$route.query.id, list };
      const res = await this.$api.pageDetailAdd(data);
      if (res) {
        this.$message.success("部署成功");
      }
    },
    async getAssApiList() {
      const res = await this.$api.getDeployResource({ type: "assApiPage" });
